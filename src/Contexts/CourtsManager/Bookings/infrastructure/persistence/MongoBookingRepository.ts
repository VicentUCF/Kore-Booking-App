...
export class MongoBookingRepository extends MongoRepository<Booking> implements BookingRepository {
  ...
  public async search(id: BookingId): Promise<Nullable<Booking>> {
    ...
    return document
      ? Booking.fromPrimitives({ court: document.court, user: document.user, date: document.date, id: id.value })
      : null;
  }
  ...
  public async searchAll(): Promise<Booking[]> {
    ...
    return documents.map(document =>
      Booking.fromPrimitives({ court: document.court, user: document.user, date: document.date, id: document._id })
    );
  }
